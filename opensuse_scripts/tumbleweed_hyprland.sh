...
sleep 5
sudo zypper install branding-openSUSE filesystem openSUSE-build-key openSUSE-release staging-build-key

echo "@patterns-base-base"
sleep 5
sudo zypper install aaa_base bash ca-certificates-mozilla coreutils coreutils-systemd glibc libnss_usrfiles2 pam pam-config purge-kernels-service rpm system-user-nobody systemd util-linux NetworkManager busybox-static elfutils glibc-locale-base hostname iproute2 issue-generator lastlog2 pam_pwquality procps shadow system-group-wheel system-user-bin system-user-daemon terminfo time timezone wtmpdb gnome-keyring upower xdg-user-dirs xdg-user-dirs-gtk lynis sysstat acct rng-tools haveged

echo "@patterns-base-documentation"
sleep 5
sudo zypper install man man-pages tealdeer

echo "@patterns-base-enhanced_base"
sleep 5
sudo zypper install aaa_base-extras acl attr bind-utils bzip2 cifs-utils cnf cpio cpupower cryptsetup curl deltarpm diffutils dmidecode dosfstools file findutils fuse gawk gettext-runtime glibc-locale gpg2 gpm grep gzip hwdata hwinfo iputils irqbalance klogd kmod krb5 less logrotate lsscsi mcelog net-snmp netcat-openbsd netcfg numactl pam-config p7zip parted pciutils perl-Bootloader perl-base pinentry plymouth prctl procinfo psmisc screen sed setserial sg3_utils smartmontools smp_utils strace sudo suse-module-tools tar tnftp usbutils wget wireless-tools wol xz unrar unzip zip zram-generator

echo "@patterns-base-x11"
sleep 5
sudo zypper install xf86-input-libinput xorg-x11-fonts-core xorg-x11-server xorg-x11-driver-video xinit xterm

echo "@patterns-base-x11_enhanced"
sleep 5
sudo zypper install xkeyboard-config dbus-1-x11 dialog fontconfig fonts-config fribidi ghostscript-x11 libdbus-glib-1-2 numlockx opensuse-welcome susepaste udev-configure-printer xauth xorg-x11-libX11-ccache fzf lsd

echo "@patterns-devel-base-devel_basis"
sleep 5
sudo zypper install git-core git-credential-libsecret go go-doc npm nodejs autoconf automake binutils bison cpp flex gawk gcc gettext-tools glibc-devel libtool m4 make patch zlib-devel binutils-devel fdupes gcc-c++ libstdc++-devel patch pkgconf-pkg-config subversion rust cargo shfmt lua-language-server lazygit fd ripgrep neovim wl-clipboard python312-neovim
...
sleep 5
sudo zypper install bc ctags diffstat indent kernel-source kernel-syms libelf-devel patchutils

echo "@patterns-devel-base-devel_rpm_build"
sleep 5
sudo zypper install libnss_usrfiles2 netcfg rpm-build
...
echo "@patterns-devel-python-devel_python3"
sleep 5
sudo zypper install python312-base python312-black python312-devel python312-flake8 python312-pip python312-pipx python312-pytest python312-requests python312-setuptools python312-tox python312-virtualenv python312-installer python312-build

echo "@graphics-drivers"
#libva-vdpau-driver
sleep 5
sudo zypper install dkms libvdpau1 libva-utils libglvnd libglvnd-devel Mesa-libva xf86-video-nv nvidia-video-G06 nvidia-driver-G06-kmp-default nvidia-gl-G06 nvidia-utils-G06 kernel-firmware-nvidia kernel-devel make acpid libglvnd libglvnd-devel kernel-firmware-amdgpu xf86-video-amdgpu ffnvcodec-devel libva-devel
...
echo "@theming"
sleep 5
sudo zypper install kvantum-manager kvantum-qt5 kvantum-qt6 kvantum-themes qt5ct qt6ct gnome-themes-extras gtk2-engine-murrine sassc

echo "@multimedia"
sleep 5
sudo zypper install ImageMagick celluloid pavucontrol pamixer pipewire pipewire-alsa pipewire-pulseaudio gstreamer-plugin-pipewire wireplumber
opi codecs

echo "@hyprland-DE"
sleep 5
sudo zypper si -d hyprland
sudo zypper install libnvidia-egl-wayland libqt5-qtwayland qt6-wayland grim slurp hypridle hyprland hyprland-wallpapers hyprpicker xdg-desktop-portal-gtk xdg-desktop-portal-hyprland xwayland wf-recorder scdoc mate-polkit nwg-look
opi swaylock-effects
#opi regreet
